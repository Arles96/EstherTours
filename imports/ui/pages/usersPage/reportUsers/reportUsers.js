...
import './reportUsers.html';

Template.reportUsers.onCreated(function createVars () {
  const date = new Date();
...
Template.reportUsers.onRendered(() => {
  const date = new Date();
...
function draw (selectedYear, templateInstance) {
  ...
  Meteor.call('reportUser', { year: Number(selectedYear) }, (error, result) => {
    if (error) {
      ...
      templateInstance.myChart.set(new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            label: `Creación de usuarios del ${this.currentYear}`,
            data: result,
            backgroundColor: [
              '#34495E',
              '#98A4A4',
