...
import './reportRestaurants.html';

Template.reportRestaurants.onCreated(function createVars () {
  const date = new Date();
...
Template.reportRestaurants.helpers({
  currentYear () {
...
Template.reportRestaurants.events({
...
Template.reportRestaurants.onRendered(() => {
  const date = new Date();
...
function draw (selectedYear, templateInstance) {
  ...
  Meteor.call('reportRestaurants', { year: Number(selectedYear) }, (error, result) => {
    if (error) {
      ...
      templateInstance.myChart.set(new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            label: `Creación de restaurantes del ${this.currentYear}`,
            data: result,
            backgroundColor: [
              '#34495E',
              '#98A4A4',
