...
import './reportTour.html';

Template.reportTour.onCreated(function createVars () {
  const date = new Date();
...
Template.reportTour.onRendered(() => {
  const date = new Date();
...
function draw (selectedYear, templateInstance) {
  ...
  Meteor.call('reportTour', { year: Number(selectedYear) }, (error, result) => {
    if (error) {
